...
class WelcomeScreen(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        grid.setSpacing(8)
        grid.addStretch()
        for label, cmd in [("system stats", "show system stats"), ("open chrome", "open chrome"),
                           ("today's calendar", "show my calendar for today"), ("check emails", "summarise my unread emails")]:
            btn = QPushButton(label)
            btn.setObjectName("quick_btn")
            btn.setCursor(Qt.PointingHandCursor)
            btn.clicked.connect(lambda checked, c=cmd: self.quick_action.emit(c))
            grid.addWidget(btn)
        grid.addStretch()
...
class ChatWidget(QWidget):
    ...
    def set_typing(self, visible: bool):
        self.typing_label.setText("_ zentra is thinking ..." if visible else "")
