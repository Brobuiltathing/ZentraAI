...
from zentra.memory import load_memory, clear_memory
from zentra.utils.seen_emails import load_seen_emails
from frontends.gui.chat_widget import ChatWidget
from frontends.gui.settings_panel import SettingsPanel
from frontends.gui.worker import WorkerThread
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        QShortcut(QKeySequence(Qt.Key_Return), self.input_box).activated.connect(self._send_message)
        QShortcut(QKeySequence(Qt.SHIFT | Qt.Key_Return), self.input_box).activated.connect(
            lambda: self.input_box.insertPlainText("\n"))
    ...
    def _handle_quick_action(self, command):
    ...
    def _check_ollama_status(self):
        try:
            ...
            if r.status_code == 200:
                ...
                self.status_text.setText(config.OLLAMA_MODEL if has_model else "model not pulled")
            else:
                self._set_offline()
        except Exception:
            self._set_offline()
