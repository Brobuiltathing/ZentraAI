...
from zentra.memory import load_memory, save_to_memory, persist_memory
from zentra.engine import process_message
from zentra.utils.seen_emails import load_seen_emails, seen_email_ids, persist_seen_emails
from zentra.actions.plugins import load_plugins
...
if GOOGLE_AVAILABLE:
    from zentra.actions.gmail import fetch_unread_emails_sync, importance_score, _format_email_digest
    ...
    async def _watcher_loop(self):
        ...
        while True:
            try:
                ...
                for ev in events:
                    await self._dm(f"**File watcher [{ev['watcher']}]:** {ev['type']} `{ev['name']}`")
            except Exception as exc:
...
if __name__ == "__main__":
    if DISCORD_BOT_TOKEN == "YOUR_BOT_TOKEN_HERE":
        print("Set DISCORD_BOT_TOKEN in config.py or .env")
        raise SystemExit(1)
