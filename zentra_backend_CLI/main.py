...
from memory import load_memory, clear_memory, USER_ID
from engine import process_message
from utils.seen_emails import load_seen_emails
from actions.plugins import load_plugins, reload_plugins, handle_plugin_list
...
def print_help():
    if not RICH_AVAILABLE:
        ...
        print("  /snapshot       context snapshot (screen + processes)")
        print("  /export [md|txt] export chat history")
        print("  /kb list|add|search  knowledge base commands")
...
def _print_offline():
    if RICH_AVAILABLE:
        console.print(f"  [{RED}]ollama offline[/] at {OLLAMA_ENDPOINT}")
    else:
        print(f"  ollama offline at {OLLAMA_ENDPOINT}")
...
def handle_slash_command(cmd: str) -> bool:
    ...
    if command in ("/quit", "/exit", "/q"):
        ...
        result = asyncio.get_event_loop().run_until_complete(
            run_action_direct("clipboard_read")
        )
        ...
        instruction = arg or ""
        result = asyncio.get_event_loop().run_until_complete(
            run_action_direct("clipboard_fix", reply=instruction)
        )
        print_bot_message(result)

    elif command == "/snapshot":
        result = asyncio.get_event_loop().run_until_complete(
            run_action_direct("context_snapshot")
        )
        print_bot_message(result)
        ...
        fmt = arg or "markdown"
        result = asyncio.get_event_loop().run_until_complete(
            run_action_direct("export_chat", app=fmt)
        )
        print_bot_message(result)
        ...
        if sub == "list":
            result = asyncio.get_event_loop().run_until_complete(run_action_direct("kb_list"))
        elif sub == "add":
            result = asyncio.get_event_loop().run_until_complete(run_action_direct("kb_add", filename=sub_arg))
        elif sub == "search":
            result = asyncio.get_event_loop().run_until_complete(run_action_direct("kb_search", app=sub_arg))
        elif sub == "clear":
            result = asyncio.get_event_loop().run_until_complete(run_action_direct("kb_clear"))
        else:
        ...
        result = handle_plugin_list({})
        print_bot_message(result)

    elif command == "/reload":
        result = reload_plugins()
        _print_msg(result, GREEN)
        _print_nl()
